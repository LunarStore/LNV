...
#include "storage/globals.hpp"
#include "storage/db.hpp"
#include "storage/file.hpp"
#include "storage/id.hpp"
...
bool service_c::upload(acl::socket_stream* conn,
	...
	logger("update file, appid: %s, userid: %s, "
...
bool service_c::download(acl::socket_stream* conn,
	...
	logger("download, appid: %s, userid: %s, fileid: %s, "
...
int service_c::save(acl::socket_stream* conn, char const* appid,
	...
	if (db.set(appid, userid, fileid, filepath, filesize) != OK) {
		error(conn, -1, "insert database fail, filid: %s", fileid);
		file.del(filepath);
		return ERROR;
	}
...
int service_c::send(acl::socket_stream* conn, char const* filepath,
	...
	char rdsnd[STORAGE_RDSND_SIZE]; // 读取写入缓冲区
	while (remain) { // 还有未可读取数据
		// 读取文件
